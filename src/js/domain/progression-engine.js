import { MASTERY_THRESHOLD, MAX_LEVEL, ROLLING_WINDOW_SIZE } from "../config/curriculum.js";

export function createDefaultProgressRecord(skillKey) {
  return {
    skillKey,
    level: 1,
    mastery: 0,
...
export class ProgressionEngine {
  applyEvaluation(progressRecord, evaluationResult, timestampIso) {
    const record = progressRecord ? { ...progressRecord } : createDefaultProgressRecord("unknown");
    ...
    if (
      record.rolling.length >= ROLLING_WINDOW_SIZE
